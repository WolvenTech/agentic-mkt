...
Field semantics defined in [ADR-006: Use Stage-Aware Agent Contracts](../../.compozy/tasks/content-quality-pipeline/adrs/adr-006.md) and enforced by stage agent configs (source of truth for each stage's worker agent). The harness [`output-schema.json`](output-schema.json) validates this structure.
...
**Pointer comment tags** (task 23+ revision, [ADR-008](../../.compozy/tasks/content-quality-pipeline/adrs/adr-008.md)):
...
**Activity tags** (task 23+ revision, [ADR-008](../../.compozy/tasks/content-quality-pipeline/adrs/adr-008.md)):
...
Local verification scripts (`pnpm green-run`, `scripts/content-quality-proof.ts`) enforce a standardized exit-code contract (per [ADR-010](../../.compozy/tasks/content-quality-pipeline/adrs/adr-010.md)) so incomplete verification is never silently reported as success.
